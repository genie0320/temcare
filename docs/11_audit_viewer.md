...
## 7. 결정된 것 (2026-08-04) — 다시 묻지 말 것

### 7-1. 접속기록 조회는 `audit_log`에 남긴다 (`access_log` 아님)

감사로그 화면에서 `access_log`를 조회하면 그 **행위**를 남긴다. 단 남기는 장부는
`audit_log`다 — `action="read"`, `target_table="access_log"`.

- `access_log`에 남기면 "내가 접속기록을 봤다"는 기록이 장부를 채워 **정작 봐야 할
  회원 개인정보 열람 기록이 묻힌다.**
- `access_log`는 `purpose`(열람 사유)가 필수라, 진단 화면을 열 때마다 사유를 타이핑해야 한다.
- `audit_log`에 행위로 남기면 추적은 되면서 접속기록 장부는 깨끗하게 유지된다.
  권한 밖 접근(`deny`)을 같은 장부에 남기는 것과 같은 논리다.

구현: `apps/audit/service.py`의 `record_access_log_view()`.
`AuditLog.ACTION_CHOICES`에 `read`를 추가했다(migration `0003`).

> 정식 운영 전 **법무 검토 때 `access_log`로 승격할지 다시 본다.** 지금 바꾸지 말 것.

### 7-2. 보관 기간("2년 보관") 문구는 화면에 넣지 않는다

**지금 로그를 파기하는 코드가 어디에도 없다.** 그런 상태에서 "2년 보관"이라고 쓰면
지키지 않는 약속을 화면에 박아두는 것이 된다.

또한 **'2년'은 접속기록(`access_log`) 기준**이라, 변경이력(`audit_log`)에 같은 숫자를
붙이면 틀린 말이 된다. 한 화면에 뭉뚱그리지 말 것.

대신 요약 탭은 **현재 상태**를 사실대로 보여준다:

| 보여주는 것 | 어디서 |
|---|---|
| 탭별(장부별) 총 건수 | `summary.audit_log.total` · `summary.access_log.total` |
| 가장 오래된 기록의 시각 | `oldest_at` (탭별로 따로) |
| 가장 최근 기록의 시각 | `latest_at` (탭별로 따로) |
| "자동 파기 미구현 — adm_030 파기 관리 예정" | `summary.purge` |

`apps/audit/tests.py::test_summary_states_purge_is_not_implemented_instead_of_promising_retention`이
응답 어디에도 `"2년"`이 없음을 못박는다 — 나중에 누가 정책 문구를 다시 넣으면 빨개진다.
보관 기간 안내는 **adm_030(파기 관리)에서 실제 파기 코드와 함께** 붙인다.
