...
## 지금 상태 (세션 인수인계 — 2026-08-01 새벽 업데이트, 체크포인트 8 완료)

**M0 완료 + M1 일곱 번째 화면(건강신호)까지 실동작.** 약점(adm_003) → 64유형(adm_002) → 영양소(adm_022) → 약재(adm_023) → 식품군(adm_025) → 혈자리(adm_026) → 건강신호(adm_007a) 전부 목록·상세 CRUD가 브라우저에서 실데이터로 끝까지 확인됐다.

- **바로 이어서 할 일**: M1 순서대로 예측질환(adm_007b)부터. 건강신호와 같은 §B 구조지만 `category` 필드가 있다(현재 UI 미노출, 모델엔 보존). 그다음은 제품(adm_027) → 요법관리(adm_024)로 M1이 끝난다.
...
- **체크포인트 5~8(약재·식품군·혈자리·건강신호)**: §C(카드 반복)와 §B(단일 레코드+약점 체크박스) 두 패턴이 예상대로 재사용됐다 — 넷 다 새 공통 컴포넌트 없이 조립만으로 끝났다. 핵심성분·효능기전처럼 반복되는 "자유텍스트+자동완성" 패턴은 `TextInput`에 `listOptions` prop을 추가해 흡수했다.
- **로컬 개발 DB 유의사항**: 화면에서 저장(카드 "통째로 교체")한 뒤 `seed_demo`를 재실행하면 카드가 중복될 수 있다 — 원인과 대처는 `docs/06_decisions.md` #21.
- **브라우저 자동화 유의사항(이 세션에서 처음 겪음)**: 헤드리스 브라우저 도구의 스크린샷 해상도(예: 800×555)와 실제 뷰포트(예: 1311×910)가 안 맞을 때가 있다 — 좌표 클릭이 엉뚱한 요소를 때릴 수 있다. 클릭이 안 먹히면 `read_page`로 `ref_N`을 얻어 `ref` 기반 클릭/`form_input`으로 바꿀 것(좌표 계산에 안 걸림).
- **테스트**: 백엔드 56개 통과(`pytest`), bandit·감사로그 우회 검사·`tsc -b`·`oxlint` 전부 클린. 다섯 화면(영양소·약재·식품군·혈자리·건강신호) 모두 브라우저로 목록→상세→저장→재조회 라운드트립 + 신규 생성 + 이미지 업로드/자동완성 + 삭제(API 직접 확인, `window.confirm`은 헤드리스 한계로 버튼 클릭까지만) 확인함.
- **아직 안 한 것**: 예측질환(adm_007b)부터 나머지 3개 콘텐츠 화면, "권한 없는 접근이 감사로그에 남는가" 체크리스트(`docs/02_architecture_constraints.md` §2)는 여전히 TODO, `window.confirm` 삭제 흐름은 헤드리스 자동화로 검증 불가(수동 확인만 가능).
...
- 약재(adm_023)·식품군(adm_025)·혈자리(adm_026)·건강신호(adm_007a) 완료. 예측질환(adm_007b)도 같은 §B 구조 — `name`·`description`·`image` + 약점 체크박스, `category`는 스키마엔 있지만 UI 미노출.
