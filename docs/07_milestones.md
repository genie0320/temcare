...
## 지금 상태 (세션 인수인계 — 2026-07-31 밤 업데이트, 체크포인트 7 완료)

**M0 완료 + M1 여섯 번째 화면(혈자리)까지 실동작.** 약점(adm_003) → 64유형(adm_002) → 영양소(adm_022) → 약재(adm_023) → 식품군(adm_025) → 혈자리(adm_026) 전부 목록·상세 CRUD가 브라우저에서 실데이터로 끝까지 확인됐다.

- **바로 이어서 할 일**: M1 순서대로 건강신호(adm_007a)부터. 혈자리보다도 단순한 §B 구조 — `name`·`note`·`image` + 약점 체크박스뿐이다. 그다음은 예측질환(adm_007b) → 제품(adm_027) → 요법관리(adm_024)로 M1이 끝난다.
...
- **체크포인트 5~7(약재·식품군·혈자리)**: §C(카드 반복)와 §B(단일 레코드+약점 체크박스) 두 패턴이 예상대로 재사용됐다 — 셋 다 새 공통 컴포넌트 없이 조립만으로 끝났다. 핵심성분·효능기전처럼 반복되는 "자유텍스트+자동완성" 패턴은 `TextInput`에 `listOptions` prop을 추가해 흡수했다.
- **로컬 개발 DB 유의사항**: 화면에서 저장(카드 "통째로 교체")한 뒤 `seed_demo`를 재실행하면 카드가 중복될 수 있다 — 원인과 대처는 `docs/06_decisions.md` #21.
- **테스트**: 백엔드 51개 통과(`pytest`), bandit·감사로그 우회 검사·`tsc -b`·`oxlint` 전부 클린. 네 화면(영양소·약재·식품군·혈자리) 모두 브라우저로 목록→상세→저장→재조회 라운드트립 + 신규 생성 + 이미지 업로드/자동완성 + 삭제(API 직접 확인, `window.confirm`은 헤드리스 한계로 버튼 클릭까지만) 확인함.
- **아직 안 한 것**: 건강신호(adm_007a)부터 나머지 4개 콘텐츠 화면, "권한 없는 접근이 감사로그에 남는가" 체크리스트(`docs/02_architecture_constraints.md` §2)는 여전히 TODO, `window.confirm` 삭제 흐름은 헤드리스 자동화로 검증 불가(수동 확인만 가능).
...
- 약재(adm_023)·식품군(adm_025)·혈자리(adm_026) 완료. 건강신호(adm_007a)는 §B 중에서도 가장 단순하다 — `name`·`note`·`image` + 약점 체크박스뿐, 위치·영상 같은 부가 필드도 없다.
