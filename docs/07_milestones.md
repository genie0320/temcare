...
## 지금 상태 (세션 인수인계 — 2026-08-01 업데이트, 체크포인트 9 완료)

**M0 완료 + M1 여덟 번째 화면(예측질환)까지 실동작.** 약점(adm_003) → 64유형(adm_002) → 영양소(adm_022) → 약재(adm_023) → 식품군(adm_025) → 혈자리(adm_026) → 건강신호(adm_007a) → 예측질환(adm_007b) 전부 목록·상세 CRUD가 브라우저에서 실데이터로 끝까지 확인됐다.

- **바로 이어서 할 일**: M1 순서대로 제품(adm_027)부터. 제품은 §B와 다르게 약점 체크박스가 없고 대신 `url`(구매 링크) 필드가 있다 — 스펙 재확인 필요. 그다음은 요법관리(adm_024)로 M1이 끝난다.
...
- **테스트**: 백엔드 61개 통과(`pytest`), bandit·감사로그 우회 검사·`tsc -b`·`oxlint` 전부 클린. 여섯 화면(영양소·약재·식품군·혈자리·건강신호·예측질환) 모두 브라우저로 목록→상세→저장→재조회 라운드트립 + 신규 생성 + 이미지 업로드/자동완성 + 삭제(API 직접 확인, `window.confirm`은 헤드리스 한계로 버튼 클릭까지만) 확인함.
- **아직 안 한 것**: 제품(adm_027)·요법관리(adm_024), "권한 없는 접근이 감사로그에 남는가" 체크리스트(`docs/02_architecture_constraints.md` §2)는 여전히 TODO, `window.confirm` 삭제 흐름은 헤드리스 자동화로 검증 불가(수동 확인만 가능).
...
- 약재(adm_023)·식품군(adm_025)·혈자리(adm_026)·건강신호(adm_007a)·예측질환(adm_007b) 완료. 제품(adm_027)은 스펙 재확인 필요 — 약점 체크박스가 없고 구매 링크(`url`) 필드가 있는 다른 구조로 추정된다.
