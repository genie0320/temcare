...
4. **개발 전용 빠른 로그인** — 소셜 로그인 없이 테스트 계정으로 진입하는 경로. `DEBUG=True`에서만 열리게 하고 프로덕션 설정에서 물리적으로 차단한다. 영업 시연에도 유용하다.
   - 관리자: `admin@ollacare.local` / `admin1234!` (`seed_demo`가 만든다). 관리자 로그인 화면의 '개발용 빠른 로그인' 버튼이 이 계정으로 들어간다.
   - **고객 확인용 상용 계정: `tem18check@example.com` / `demo1234!check` (체질 `TEM18`).**
     고객 화면을 확인할 때마다 가입 동선을 다시 타지 않도록 **계속 재사용한다**(2026-08-03 확정).
     `dev-login`은 관리자 계정으로만 들어가므로 고객 화면은 이 계정으로 로그인할 것.
     ★ 이 계정은 로컬 DB에만 있다. `seed_demo`가 만들지 않으므로 DB를 새로 만들면 다시 가입해야 한다.
...
- 원장 등 외부에 시연이 필요할 때는 **`make demo`** 로 로컬 개발 서버를 그 순간만 공개 HTTPS 주소로 노출한다(Cloudflare Tunnel, `scripts/demo.py`). ★ **`PUBLIC_DEMO=True`로 뜨며, 이때 `dev-login`(무인증 관리자 진입)이 닫힌다** — 터널 시연도 DEBUG로 돌기 때문에 이 구분이 없으면 링크를 아는 누구나 관리자가 된다(결정 #37). 터널은 고객 앱만 가리키고 관리자는 열리지 않는다. 포트포워딩·공인 IP 확보가 필요 없고(터널링 서비스로 나가는 연결이라 가정용 회선의 CGNAT 문제를 안 탄다), 시연이 끝나면 끄면 된다. 비용 0원.
